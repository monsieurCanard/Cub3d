...
/*   Updated: 2024/05/16 00:27:26 by jbrousse         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_3d_pov(float dist_ray, int color, int i)
{
	...
	line_height = 32 * 800 / dist_ray;
	if (line_height > 800)
		line_height = 800;
	start = vector2(i * 13, 400 - line_height / 2);
	while (start.y < line_height + 400 - line_height / 2)
	{
		start.x = i * 13;
...
int	update_game(void *obj)
{
	...
	while (i < 60)
	{
		if (angle > 2 * PI)
			...
			angle += 2 * PI;
		ray = get_ray(data, angle, i, &color);
