...
/*   Updated: 2024/06/07 10:52:12 by Monsieur_Ca      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	is_sourrounded(t_map *map_data, int *nb_player)
{
	...
	i = 1;
	if (!(valid_border(map_data->map[0]))
		|| !(valid_border(map_data->map[map_data->size_z - 1])))
		return (MAP_HOLE);
	while (map_data->map[i] != NULL)
	{
		...
		{
			ret = valid_condition(map_data->map, i, j);
			if (ret != SUCCESS)
				return (ret);
			else if (is_a_player(map_data->map[i][j]) == true)
				*nb_player += 1;
			j++;
		}
		i++;
